...
(async () => {
  ...
  try {
    ...
    let clickCount = 0;
    const maxClicks = 3; // Set the maximum number of clicks
    ...
    const cardData = await page.evaluate(() => {
      ...
      return cards.map((card) => {
        ...
        const room = card.querySelector(
          ".field--name-field-event-room"
        ).textContent;
        const access = card.querySelector(
          ".field--name-field-event-access"
        ).textContent;
  ...
  `;
    ...
    fs.writeFileSync("meetingsss.js", jsData.trim());
    console.log("Data saved to meetings.js");
